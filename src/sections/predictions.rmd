...
# p_study_count <- stop_predictions %>%
#     filter(start_date <= today()) %>%
#     filter(year(start_date) > 1990) %>%
#     group_by(prediction) %>%
#     summarise(count = n()) %>%
#     mutate(prediction = fct_reorder(prediction, count)) %>%
#     mutate(comparison = "Total studies") %>%
...
stop_to_plot <- bind_rows(
    data_to_plot %>%
        ...
        mutate(year = fct_other(as.factor(year), keep = as.character(seq(1995, 2021)))) %>%
...
p_meta_tiles <- data_to_plot %>%
    ...
    # values = rev(colorRampPalette(rev(RColorBrewer::brewer.pal(11,'Spectral')))(7)),
    # values = c("#9E0142", "#E95D46", "#FDBE6F", "#ffff3e", "#BEE4A0", "#54AEAD", "#5E4FA2"),
    values = c("#E64B35FF", "#F39B7FFF", "#533377", "#3C5488FF", "#00a048", "#815d34", "grey30"),
    breaks = c("Negative", "Safety", "Suspicious", "Neutral", "Success", "Invalid", "Unclassified")) +
    theme_cowplot(font_size = 12) +
...
p_predictions_tiles <- stop_to_plot %>%
    ...
    scale_fill_manual(
        ...
        # values = colorRampPalette(rev(RColorBrewer::brewer.pal(11,'Spectral')))(length(levels(stop_to_plot$interval))),
...
p_supertile <- plot_grid(
    p_meta_tiles + theme(legend.position = "none", plot.margin = margin(l = 30, t = 5, r = 0, unit = "pt")),
    p_predictions_tiles + theme(legend.position = "none", axis.text.y = element_blank()),
...
p_predictions_by_phase <- data_to_plot %>%
    ...
    ggplot(aes(x = str_wrap(phase, width = 10), y = stopRatio, fill = fct_rev(prediction))) +
...
```{r, predictions_by_therapy_summary}
...
test <- clinical %>%
    ...
    summarise(
        ...
        mostRecentStopped = as.Date(max(studyStartDate[!is.na(why_stopped)], na.rm = TRUE)),
    ) %>%
