...
toplot_safety_main <- all_tests %>%
    filter(predictionType == "metareason") %>%
    filter(prediction == "Safety_Sideeffects") %>%
    filter(comparisonType %in% c("ta", "geneticConstrain", "rnaSpecificity", "lof_tolerance")) %>%
    ...
    mutate(comparisonType = fct_rev(fct_reorder(comparisonType, estimate, max, na.rm = TRUE))) %>%
    mutate(prediction = fct_recode(prediction, "Trials stopped because of Safety" = "Safety_Sideeffects")) %>%
    mutate(oddsinterval = paste(round(estimate, digits = 3), " (", round(conf.low, digits = 2), "-", round(conf.high, digits = 2), ")", sep = "")) %>%
...
p_safety_main_numbers <- toplot_safety_main %>%
    ...
    select(comparison, comparisonType, prediction, a, oddsinterval, significance) %>%
